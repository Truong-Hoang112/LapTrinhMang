...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let board, rows, cols, cellSize, mySymbol = '', currentPlayer = '', isGameReady = false, gameMode = 'normal';
    ...
    function drawPieces() {
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                ...
                if (!piece) continue;
                const centerX = c * cellSize + cellSize / 2, centerY = r * cellSize + cellSize / 2;
                ctx.beginPath();
                if (piece === 'X') {
                    ctx.strokeStyle = '#d9534f'; const margin = cellSize * 0.15;
                    ctx.moveTo(centerX - margin, centerY - margin); ctx.lineTo(centerX + margin, centerY + margin);
                    ctx.moveTo(centerX + margin, centerY - margin); ctx.lineTo(centerX - margin, centerY + margin);
                } else {
    ...
    function updateStatus() {
        if (!isGameReady) { statusEl.textContent = `Phòng: ${roomId}. Đang chờ...`; return; }
        const opponentName = (gameMode === 'ai') ? 'Máy' : 'Đối thủ';
        const turnText = (currentPlayer === mySymbol) ? '>>> Lượt của BẠN <<<' : `Lượt của ${opponentName}`;
    ...
    socket.on('playerUpdate', (data) => {
        gameMode = data.mode;
        const humanPlayers = Object.keys(data.players).filter(id => id !== 'ai_player').length;
        isGameReady = (gameMode === 'ai' && humanPlayers >= 1) || (gameMode !== 'ai' && humanPlayers === 2);
    ...
    socket.on('gameOver', (message) => {
        ...
        setTimeout(() => { alert(message); statusEl.textContent = message; }, 150);
    });
    ...
    socket.on('roomFull', (message) => { alert(message); window.location.href = '/home.html'; });
    socket.on('roomError', (message) => { alert(message); window.location.href = '/home.html'; });
    ...
    if (surrenderBtn) {
        surrenderBtn.addEventListener('click', () => {
            if (confirm('Bạn có chắc muốn đầu hàng?')) socket.emit('surrender', roomId);
        });
    }
    ...
    if (roomId && requestedSize) {
        ...
        socket.emit('joinRoom', {
            roomId, size: requestedSize, username: user.username, mode: requestedMode || 'normal'
        });
    ...
    socket.on('connect', () => console.log('Kết nối Socket.IO thành công:', socket.id));
